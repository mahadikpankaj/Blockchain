...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $BLACKLISTED_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      echo "ERROR! Local Fabric binary version of $LOCAL_VERSION does not match this newer version of pankaj_property_registration_network and is unsupported. Either move to a later version of Fabric or checkout an earlier version of fabric-samples."
      exit 1
    fi

    echo "$DOCKER_IMAGE_VERSION" | grep -q $UNSUPPORTED_VERSION
    if [ $? -eq 0 ]; then
      echo "ERROR! Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match this newer version of pankaj_property_registration_network and is unsupported. Either move to a later version of Fabric or checkout an earlier version of fabric-samples."
      exit 1
    fi
  done
  preReq=true
...
function setCACertificatesPath(){
  if [ -d "crypto-config" ]; then
    export CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/registrar.property-registration-network.com/ca && ls *_sk)
    export CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/users.property-registration-network.com/ca && ls *_sk)
  fi
}
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.property-registration-network.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.property-registration-network.com
    docker cp -a orderer.property-registration-network.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.property-registration-network.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.property-registration-network.com

    for PEER in peer0.registrar.property-registration-network.com peer1.registrar.property-registration-network.com peer0.users.property-registration-network.com peer1.users.property-registration-network.com; do
...
# (``peer0.registrar.property-registration-network.com`` & ``peer0.users.property-registration-network.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/registrarMSPanchors.tx -channelID $CHANNEL_NAME -asOrg registrarMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for registrarMSP..."
    exit 1
  fi
  ...
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/usersMSPanchors.tx -channelID $CHANNEL_NAME -asOrg usersMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for usersMSP..."
    exit 1
  fi
  echo
